...
const Scene = () => {
  ...
  const { material, uniforms } = useMemo(() => {
    ...
    const flow = oneMinus(
      smoothstep(0, 0.02, abs(tDepthMap.sub(uProgress)))
    );
...
export default function Hero3D() {
  ...
  return (
    <div className="relative h-svh overflow-hidden">
      {/* TEXT */}
      <div className="pointer-events-none absolute inset-0 z-10 flex flex-col items-center justify-center text-center uppercase">
        <div className="text-4xl md:text-6xl xl:text-7xl font-extrabold text-white flex gap-3">
        ...
        <p
          className={`mt-4 text-sm md:text-xl font-medium font-sans text-white ${
            subtitleVisible ? 'fade-in-subtitle' : ''
          }`}
          style={{
            animationDelay: `${titleWords.length * 0.13 + subtitleDelay}s`,
